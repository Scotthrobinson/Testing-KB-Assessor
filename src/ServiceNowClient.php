...
final class ServiceNowClient
{
     ...
     */
    public function fetchUpdatedArticles(?string $sinceIso = null): array
    {
        $all = [];
        ...
        do {
            $params = [
                'sysparm_query' => $this->buildQuery($sinceIso),
    ...
    private function buildQuery(?string $sinceIso): string
    {
        $filters = [];
        if ($this->defaultQuery !== '') {
            $filters[] = $this->defaultQuery;
