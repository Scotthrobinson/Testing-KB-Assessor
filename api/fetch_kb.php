...
try {
    ...
    if ($overrideSince !== null) {
        ...
        // Full fetch requested - fetch everything (no date filter)
        $since = null;
    } else {
    ...
    // Fetch articles - pass $full flag to ServiceNowClient so it can adjust query
    error_log('[fetch_kb] Fetching articles - full: ' . ($full ? 'true' : 'false') . ', since: ' . ($since ?? 'null'));
    $articles = $serviceNow->fetchUpdatedArticles($since, $full);
    error_log('[fetch_kb] Fetched ' . count($articles) . ' articles from ServiceNow');
